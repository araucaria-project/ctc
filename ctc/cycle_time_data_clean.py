...
class CycleTimeDataClean(AbstractCycleTime):
            ...
            for p_no, q_data in enumerate(data, start=0):
                if m_dict['started'] < p_no < m_dict['ended']:
                    ...
                    if 'done' not in q_data.keys():
                        if q_data['command_name'] not in AbstractCycleTime.NO_TRAIN_COMMANDS:
                            if data[p_no + 1]['skipped'] == 'False':
                                dat = CycleTimeDataClean._build_command_data(data=data,
                                                                             telescope=telescope,
                                                                             night_id=n_id,
                                                                             line_start=p_no)
                                if dat is not None:
    ...
    def _verify_nights(full_nights_id: Dict[str, Dict[str, Any]], telescope: str, base_folder: str):
        ...
        if f is not None:
            ...
            for n in l:
                if len(n) > 1:
                    d = CycleTimeDataClean.decode_data(n)
    ...
    def _save_commands_data_to_files(commands_data: Dict[str, Any],
        ...
        for n in commands_data.keys():
            for m in commands_data[n]:
                CycleTimeDataClean.add_to_file(
                    data=CycleTimeDataClean.encode_data(m),
                    folder=base_folder,
        ...
        for p, q in full_nights_id.items():
            ...
            CycleTimeDataClean.add_to_file(
                data=CycleTimeDataClean.encode_data(data=dat),
                folder=base_folder,
                file_name=CycleTimeDataClean.night_verif_file_name(telescope=telescope))

    @staticmethod
    def _build_command_data(data: List[Dict[str, Any]], telescope: str, night_id: str, line_start: int) -> Dict[str, Any]:
        ...
        if third['command_name'] == 'NIGHTPLAN' and 'done' not in third.keys() and 'skipped' not in third.keys():
            ...
            try:
                dat = OrderedDict({
                    CycleTimeDataClean.DATA_CLEAN_ORDER[0]:
                        night_id,
                    CycleTimeDataClean.DATA_CLEAN_ORDER[1]:
                        first['command_name'],
                    CycleTimeDataClean.DATA_CLEAN_ORDER[2]:
    ...
    @staticmethod
    def data_clean(telescope: str, base_folder: str) -> None:
        """
    ...
    @staticmethod
    def data_clean_all(base_folder: str) -> None:
        ...
        for t in tel:
            ...
            CycleTimeDataClean.data_clean(telescope=t, base_folder=base_folder)
